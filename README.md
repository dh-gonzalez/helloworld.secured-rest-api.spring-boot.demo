...
### Docker
* `docker run --name helloworld-container -d -p 443:443 -v {absolute/path/to/config}:/config {absolute/path/to/keystore}:/keystore -e SPRING_CONFIG_LOCATION=/config/ ghcr.io/dh-gonzalez/helloworld.secured-rest-api.spring-boot.demo:latest`
...
* `curl --silent --insecure https://localhost/api/v1/anonymous/greeting` should answer `Hello, World!`
* `curl --silent --insecure https://localhost/api/v1/secured/greeting?name=David` should answer `Hello, David!`
